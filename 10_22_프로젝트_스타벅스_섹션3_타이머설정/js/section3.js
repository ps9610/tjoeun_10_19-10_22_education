...
//4-2. prev
    $(".prev-btn").on({ //뒤 이벤트 리스너 on(power on할 때 그 의미)
        ...
        $(".pause-play-btn").addClass("addPlay");//버튼을 누르는 이벤트가 발생하면 prev함수 내용이 연결됨
        //만약 toggle 변수 사용했으면 t=1; 써서 중지상태임을 알려줘야됨
...
initTimerFn(); //브라우저 키자마자 바로 실행함, //함수로 만들어준 이유 : 저 메모리 번호가 랜덤이라 어떤 숫자가 될지 몰라서 항상 같은 숫자가 대입되라고
/*
