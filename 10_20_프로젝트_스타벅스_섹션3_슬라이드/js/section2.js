(function(window, document, $){
    ...
    function noticeSlideFn(){
        ...
        //👆 모두 초기화 24픽셀 아래에서 대기
        $(".notice-list li").eq(cnt).stop().animate({top:0},0)/* 기다리는 요소들은 무조건 0초해야 일시정지함 */.css({zIndex:1}); //롤링 대상이 되는 요소를 앞에 쓰면됨
        //👆 첫번째만 화면에서 대기
        $(".notice-list li").eq(cnt+1>4? 0:cnt).stop().animate({top:24},0)/* top 24에서 일시정지 */.animate({top:0},1000/* 올라오는데 걸리는 시간 1초 */).css({zIndex:3}); //롤링 대상이 되는 요소를 앞에 쓰면됨
    ...
    */
    ...
    $(".promotion-btn").on({
        click:function(e){
            ...
            $("#section3").stop().slideToggle(500); //animate와 똑같이 버블링 발생할수있음 //toggle만들기(슬라이드 다시 올려야되니까)
        }
        });
